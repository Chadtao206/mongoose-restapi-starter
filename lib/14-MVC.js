...
const loginView = `
{{! Unstyled Login View boilerplate is completed for you }}
...
const signupView = `
{{! Unstyled Signup view boilerplate is completed for you }}
...
{{! inline script to handle click of logout button }}
<script>
  const logout = async function() { const response = await
  fetch('/api/user/logout', { method: 'POST', headers: { 'Content-Type':
  'application/json' }, }); if (response.ok) { document.location.replace('/'); }
...
{{! A boilerplate dashboard layout is completed for you, feel free to modify and style to your liking }}
...
<html lang="en">
  ...
  <body>
    ...
    <header>
      <h1><a href="/dashboard">Your Dashboard</a></h1>
    </header>
...
const adminPage = `
{{!-- TODO - write the template to show all the user's posts on the /dashboard route --}}
<h1>This is the dashboard main page that should display all of a users posts</h1>
`;

const homePage = `
{{!-- TODO - write the template to show all posts on the / route --}}
<h1>This is the home page that displays posts of all users</h1>
`;
...
export default {
  ...
  files: [
    ...
    { path: "views/admin-all-posts.handlebars", content: adminPage },
    { path: "views/homepage.handlebars", content: homePage },
  ],
  message:
    "All done!✨ Your project is ready to go.\nStart by running the schema.sql in db directory and add your environment variables to the .env file\n then type npm start to run the application\n Happy coding!",
};
